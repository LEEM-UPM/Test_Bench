...
void prueba_sin_sensores_lenta() {
...
void paquete_10Hz() {
  ...
  t = t * 0.001f;
  almacena_dato_float(&t, &(Paquete[2]), 'u');
  ...
  almacena_dato_float((&fb), &(Paquete[42]), 'h');
  Paquete[47] = 'E';
  Paquete[48] = XORChecksum8(Paquete, 47);
...
void paquete_100Hz() {
  ...
  t = t * 0.001f;
  almacena_dato_float(&t, &(Paquete[2]), 'v');

  float fc = analogRead(A0);

  almacena_dato_float((&fc), &(Paquete[7]), 'a');
  Paquete[12] = 'E';
  Paquete[13] = XORChecksum8(Paquete, 12);
